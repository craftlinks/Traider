...
class IBApp(EClient, EWrapper):
    ...
    def error(
        ...
        print(
            f"reqId: {reqId}, errorCode: {errorCode}, errorString: {errorString}, orderReject: {advancedOrderRejectJson}"
        )
    ...
    def openOrder(self, orderId, contract, order, orderState):
        print(
            f"Open order: {orderId}, contract: {contract}, order: {order}, orderState: {orderState}"
        )
    ...
    def orderStatus(
        ...
        print(
            f"Order status: {orderId}, status: {status}, filled: {filled}, remaining: {remaining}, avgFillPrice: {avgFillPrice}, permId: {permId}, parentId: {parentId}, lastFillPrice: {lastFillPrice}, clientId: {clientId}, whyHeld: {whyHeld}, mktCapPrice: {mktCapPrice}"
        )
...
def main():
    ...
    app.connect(
        "127.0.0.1", port, 0
    )  # Requires IB Gateway/TWS to be running on your local machine
