...
from ibapi.protobuf.HistoricalTickBidAsk_pb2 import (
    HistoricalTickBidAsk as HistoricalTickBidAskProto,
)
from ibapi.protobuf.HistoricalTickLast_pb2 import (
    HistoricalTickLast as HistoricalTickLastProto,
)
from ibapi.protobuf.HistogramDataEntry_pb2 import (
    HistogramDataEntry as HistogramDataEntryProto,
)
from ibapi.protobuf.HistoricalDataBar_pb2 import (
...
def decodeContract(contractProto: ContractProto) -> Contract:
    ...
    if contractProto.HasField("primaryExch"):
        contract.primaryExch = contractProto.primaryExch
...
def decodeOrder(
    ...
    if orderProto.HasField("deltaNeutralSettlingFirm"):
        order.ddeltaNeutralSettlingFirm = orderProto.deltaNeutralSettlingFirm
    ...
    order.smartComboRoutingParams = decodeTagValueList(
...
def setConditionFields(
    ...
    if orderConditionProto.HasField("isConjunctionConnection"):
        orderCondition.isConjunctionConnection = (
            orderConditionProto.isConjunctionConnection
        )


@staticmethod
def setOperatorConditionFields(
    orderConditionProto: OrderConditionProto, operatorCondition: OperatorCondition
):
...
def setContractConditionFields(
    orderConditionProto: OrderConditionProto, contractCondition: ContractCondition
):
...
def decodeContractDetails(
    ...
    if contractDetailsProto.HasField("suggestedSizeIncrement"):
        contractDetails.suggestedSizeIncrement = Decimal(
            contractDetailsProto.suggestedSizeIncrement
        )
    ...
    if contractDetailsProto.HasField("fundBackLoadTimeInterval"):
        contractDetails.fundBackLoadTimeInterval = (
            contractDetailsProto.fundBackLoadTimeInterval
        )
    ...
    if contractDetailsProto.HasField("fundClosedForNewInvestors"):
        contractDetails.fundClosedForNewInvestors = (
            contractDetailsProto.fundClosedForNewInvestors
        )
    if contractDetailsProto.HasField("fundClosedForNewMoney"):
        contractDetails.fundClosedForNewMoney = (
            contractDetailsProto.fundClosedForNewMoney
        )
    ...
    if contractDetailsProto.HasField("fundMinimumInitialPurchase"):
        contractDetails.fundMinimumInitialPurchase = (
            contractDetailsProto.fundMinimumInitialPurchase
        )
    if contractDetailsProto.HasField("fundMinimumSubsequentPurchase"):
        contractDetails.fundSubsequentMinimumPurchase = (
            contractDetailsProto.fundMinimumSubsequentPurchase
        )
    ...
    if contractDetailsProto.HasField("fundBlueSkyTerritories"):
        contractDetails.fundBlueSkyTerritories = (
            contractDetailsProto.fundBlueSkyTerritories
        )
    ...
    if contractDetailsProto.HasField("fundDistributionPolicyIndicator"):
        contractDetails.fundDistributionPolicyIndicator = getEnumTypeFromString(
            ...
            contractDetailsProto.fundDistributionPolicyIndicator,
        )
    if contractDetailsProto.HasField("fundAssetType"):
        contractDetails.fundAssetType = getEnumTypeFromString(
            FundAssetType, contractDetailsProto.fundAssetType
        )
...
def decodeHistoricalTickBidAsk(
    historicalTickBidAskProto: HistoricalTickBidAskProto,
) -> HistoricalTickBidAsk:
...
def decodeHistoricalTickLast(
    historicalTickLastProto: HistoricalTickLastProto,
) -> HistoricalTickLast:
