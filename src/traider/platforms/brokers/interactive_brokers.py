...
class IBApp(EWrapper, EClient):
    ...
    def openOrder(
        self, orderId: int, contract: IBContract, order: IBOrder, orderState: OrderState
    ):
        ...
        app_order = Order(
            ...
            if order.lmtPrice is not None and order.lmtPrice != UNSET_DOUBLE
            else None,
            order_id=orderId,
    ...
    def orderStatus(
        ...
        logger.info(
            "OrderStatus. Id: %s, Status: %s, Filled: %s, Remaining: %s, AvgFillPrice: %s",
            orderId,
            status,
            filled,
            remaining,
    ...
    def tickPrice(self, reqId, tickType, price, attrib):
        ...
        if reqId in self.market_data:
            ...
            if (
                self.market_data[reqId].bid_price is not None
                and self.market_data[reqId].ask_price is not None
            ):
                if reqId in self.market_data_events:
                    self.market_data_events[reqId].set()
...
class InteractiveBrokersPlatform(TradingPlatform):
    ...
    def get_market_data(
        ...
        self.app.reqMktData(
            ticker_id, ib_contract, generic_tick_list, snapshot, regulatory_snapshot, []
        )
        ...
        if data and data.bid_price is not None and data.ask_price is not None:
            ...
            logger.warning(
                "Market data for %s received but incomplete.", contract.symbol
            )
            return None
    ...
    def _place_order(
        self, contract: Contract, order: IBOrder, order_id: int | None = None
    ) -> int:
        """Creates the IB contract and places the order. Returns the order id used."""
        ib_contract = self._create_ib_contract(contract)
        order_id_to_use = (
            order_id if order_id is not None else self._get_next_order_id()
        )
