...
def save_earnings_data_to_db(events: list[EarningsEvent], conn: sqlite3.Connection, *, max_retries: int = 3) -> List[int]:
    ...
    This routine now delegates the heavy lifting to the instance level
    :pymeth:`EarningsEvent.to_db`. The batching / retry logic from the old
    implementation is preserved for performance and robustness.
    """
