...
class GlobeNewswirePoller(AtomFeedPoller):
    ...
    def parse_feed_entries(self, xml_content: bytes) -> List[BaseItem]:
        ...
        for entry in entries:
            ...
            subjects = [sel.text.strip() for sel in subject_els if sel.text and sel.text.strip()] or None
            keywords = [kel.text.strip() for kel in keyword_els if kel.text and kel.text.strip()] or None
            ...
            items.append(GlobeNewswireItem(
                id=entry_id,
                title=title,
                url=href,
