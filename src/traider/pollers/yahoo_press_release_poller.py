...
class YahooPressReleasePoller(Poller[PressRelease]):
    def __init__(
        self, tickers: list[str], interval: int = 1, *, use_db: bool = True
    ) -> None:
        config = PollerConfig.from_env("YPR", default_interval=interval)
        self.config = config
        ...
        if self.use_db:
            try:
                from traider.db.database import (
                    get_db_connection,
                    create_tables,
                ...
                logger.exception(
                    "Failed to initialise database – continuing with cache-only mode: %s",
                    db_exc,
                )
                self.use_db = False
        ...
        try:
            while not self._shutdown_event.is_set():
                ...
                valid_press_releases = [
                    pr for pr in all_press_releases if pr.url is not None
                ]

                # 3. Persist new events and get them back
                if valid_press_releases:
                    newly_persisted = await self._filter_and_persist_new(
                        valid_press_releases
                    )
                    if newly_persisted:
    ...
    async def _filter_and_persist_new(
        ...
        for pr in press_releases:
            last_seen_url: str | None = self.cache.get(
                f"ypr:last_url:{pr.ticker.upper()}"
            )
        ...
        if self.use_db:
            try:
                ...
                logger.error(
                    "Database error while saving press release for %s: %s",
                    new_press_releases[0].ticker,
                    db_exc,
                )
                return []
    ...
    async def _emit(self, new_press_releases: list[PressRelease]) -> None:
        ...
        logger.debug(
            "[NEW] %s | %s",
            new_press_releases[0].ticker.upper(),
            new_press_releases[0].title,
        )
        ...
        if self._sink is not None:
            try:
                ...
                logger.exception(
                    "Sink emitted an error for %s: %s",
                    new_press_releases[0].url,
                    sink_exc,
                )
