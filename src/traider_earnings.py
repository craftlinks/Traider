...
class MessageBroker(Protocol[Message]):
    async def publish(self, channel_name: Channel, message: Message) -> None:
        ...

    async def subscribe(self, channel_name: Channel) -> asyncio.Queue[Message]:
        ...

    def unsubscribe(self, channel_name: str, queue: asyncio.Queue[Message]) -> None:
        ...


class InMemoryBroker(MessageBroker[Message]):
    def __init__(self):
        self.channels = defaultdict(list[asyncio.Queue[Message]])
...
async def worker_loop(worker_name: str, queue: asyncio.Queue[Message], shutdown_event: asyncio.Event, worker_fn: Callable[[Message], Awaitable[None]]):
    while True:
        try:
...
async def cpu_bound_worker_1(
    ...
    broker: MessageBroker[EarningsEvent],
    ...
    async def process_event(event: EarningsEvent):
...
async def test_earnings_producer(mbr: MessageBroker[EarningsEvent], shutdown_event: asyncio.Event, startup_barrier: asyncio.Barrier):
    ...
    while not shutdown_event.is_set():
        ...
        await mbr.publish(Channel.EARNINGS, earnings_event)
...
async def test_earnings_worker(mbr: MessageBroker[EarningsEvent], shutdown_event: asyncio.Event, startup_barrier: asyncio.Barrier):
    
    queue = await mbr.subscribe(Channel.EARNINGS)

    await startup_barrier.wait()
    
    async def worker_fn(message: EarningsEvent):
        pprint.pprint(f"worker 1: Received earnings event: {message}")
        if message.id > MAX_EVENTS:
            mbr.unsubscribe(Channel.EARNINGS, queue)
            shutdown_event.set()
            return
        await asyncio.sleep(1)
    
    await worker_loop("test_earnings_worker_1", queue, shutdown_event, worker_fn)
...
async def main():
    ...
    with ProcessPoolExecutor(max_workers=cpu_cores) as process_pool:
        await asyncio.gather(
            test_earnings_producer(mbr, shutdown_event, startup_barrier),
            test_earnings_worker(mbr, shutdown_event, startup_barrier),
            cpu_bound_worker_1(process_pool, mbr, shutdown_event, startup_barrier)
        )
