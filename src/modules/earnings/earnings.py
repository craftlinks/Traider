...
for name in ("LiteLLM", "openai", "httpx", "dspy", "crawlee.crawlers._playwright._playwright_crawler"):
...
class SuppressCrawlee403Filter(logging.Filter):
    def filter(self, record: logging.LogRecord) -> bool:  # type: ignore[override]
        ...
        if "Assuming the session is blocked based on HTTP status code 403" in message:
            ...
            if url_snippet:
                logger.info("Crawler blocked (403) - suppressed stacktrace: %s", url_snippet)
            else:
        ...
        contains_unknown_host = (
            "NS_ERROR_UNKNOWN_HOST" in message
            or (
                record.exc_info
        ...
        if is_retries_msg or contains_unknown_host:
            ...
            if url_snippet:
                logger.info("Crawler navigation issue (%s) - suppressed stacktrace: %s", reason, url_snippet)
            else:
                logger.info("Crawler navigation issue (%s) - suppressed stacktrace", reason)
            return False
        return True

# Attach the filter to Crawlee loggers that emit the message
_crawlee_playwright_logger = logging.getLogger("crawlee.crawlers._playwright._playwright_crawler")
    ...
    with get_db_connection() as db_conn:
        for ticker in tickers:
            ...
            if not press_releases_urls:
                ...
                logger.info(f"Press releases urls found in our database for {ticker}: {press_releases_urls}: GO POLLING!!")
