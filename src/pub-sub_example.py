...
def _cpu_bound_worker_fn(message: EarningsEvent) -> None:
    logger.info(f"Also received earnings event for company: {message.company_name}")
    time.sleep(1)
    return None
...
async def earnings_press_release_poller(
    router: MessageRouter, company_name: str, ticker: str
):
    ...
    try:
        ...
        logger.debug(
            f"[earnings_press_release_poller] Cancelled for {company_name} – ttl expired"
        )
